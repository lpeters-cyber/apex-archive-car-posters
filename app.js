...
const starterPosters=[{
  ...
  brand:"Ferrari",
  model:"F8 Tributo",
  description:"A minimalist technical tribute to Ferrari’s 2019 mid-engined supercar. Finished in dramatic gloss black, the F8 Tributo pairs sculpted aerodynamics with a 3.9-litre twin-turbo V8 producing 720 PS. The poster balances a clean side profile with blueprint views and key performance figures, celebrating a machine built for both everyday usability and extraordinary speed.",
  extras:["2019","3.9L Twin-Turbo V8","720 PS","770 Nm","0–100 km/h: 2.9 s","340 km/h","7-Speed Dual-Clutch","Rear-Wheel Drive"],
...
starterPosters.push(...(window.koenigseggCatalog||[]),...(window.porscheCatalog||[]),...(window.f1Catalog||[]),...(window.racetrackCatalog||[]),...(window.bikeCatalog||[]),...(window.paganiCatalog||[]));
const savedPosters=localStorage.getItem("apex-posters");
const savedList=savedPosters===null?null:JSON.parse(savedPosters), seedVersion="17";
...
const comboSets={
  bugatti:{name:"Bugatti Legacy · 5-poster set",ids:["bugatti-eb110-1991","bugatti-veyron-2005","bugatti-chiron-2016","bugatti-mistral-poster-2","bugatti-tourbillon-poster-1"]},
  ...
  motorcycles:{name:"Motorcycle Icons · 12-poster collection",ids:["bike-suzuki-hayabusa-2024","bike-kawasaki-ninja-h2r-2024","bike-triumph-speed-triple-1200-rs-2024","bike-ducati-panigale-v4-r-black-2024","bike-honda-fireblade-sp-2024","bike-yamaha-yzf-r1m-2024","bike-bmw-m-1000-rr-2024","bike-bmw-r-1300-gs-adventure-2024","bike-yamaha-yzf-r1-2024","bike-kawasaki-ninja-zx-10r-2024","bike-ducati-streetfighter-v4-sp2-2024","bike-ducati-panigale-v4-r-red-2024"]},
...
els.detail.onclick=e=>{if(e.target.classList.contains("detail-close"))els.detail.close();if(e.target.dataset.download)downloadPoster(e.target.dataset.download,e.target);if(e.target.dataset.delete){if(confirm("Delete this poster from your archive?")){posters=posters.filter(p=>p.id!==e.target.dataset.delete);save();els.detail.close();render()}}};
