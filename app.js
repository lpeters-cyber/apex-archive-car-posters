...
brands.Yamaha=["YZF-R1M","YZF-R1","YZF-R6"];
...
starterPosters.push(...(window.koenigseggCatalog||[]),...(window.porscheCatalog||[]),...(window.f1Catalog||[]),...(window.racetrackCatalog||[]),...(window.bikeCatalog||[]),...(window.paganiCatalog||[]),...(window.mclarenCatalog||[]),...(window.lamborghiniCatalog||[]),...(window.july5Catalog||[]));
const savedPosters=localStorage.getItem("apex-posters");
const savedList=savedPosters===null?null:JSON.parse(savedPosters), seedVersion="19";
...
const comboSets={
  ...
  motorcycles:{name:"Motorcycle Icons Â· 15-poster collection",ids:["bike-suzuki-hayabusa-2024","bike-kawasaki-ninja-h2r-2024","bike-triumph-speed-triple-1200-rs-2024","bike-ducati-panigale-v4-r-black-2024","bike-honda-fireblade-sp-2024","bike-yamaha-yzf-r1m-2024","bike-bmw-m-1000-rr-2024","bike-bmw-r-1300-gs-adventure-2024","bike-yamaha-yzf-r1-2024","bike-kawasaki-ninja-zx-10r-2024","bike-ducati-streetfighter-v4-sp2-2024","bike-ducati-panigale-v4-r-red-2024","yamaha-yzf-r1m-2024-black-red","yamaha-yzf-r1-2024-black-red","yamaha-yzf-r6-2024-black-red"]},
  ...
  ,astonmartin:{name:"Aston Martin Performance Collection · 6-poster collection",ids:["aston-martin-vantage-2023-green","aston-martin-db12-2023-green","aston-martin-valhalla-2024-green","aston-martin-valkyrie-2023-green","aston-martin-dbs-superleggera-2023-green","aston-martin-vanquish-2023-green"]}
  ,bmw:{name:"BMW M Collection · 4-poster collection",ids:["bmw-m3-e46-2001-blue","bmw-m3-e36-1992-green","bmw-m4-f82-2015-yellow","bmw-1m-coupe-2008-orange"]}
};
...
function render(){
  ...
  els.grid.innerHTML=shown.map(p=>`<article class="poster-card" data-id="${p.id}"><div class="poster-image"><img src="${p.image}" alt="${escapeHtml(p.brand+' '+p.model+' premium technical poster artwork by Throttle Glory')}" loading="lazy" decoding="async"></div><div class="poster-info"><span class="poster-brand">${escapeHtml(p.brand)}</span><h3>${escapeHtml(p.model)}</h3><p>${escapeHtml(p.description)}</p>${p.extras.length?`<div class="tags">${p.extras.slice(0,3).map(x=>`<span class="tag">${escapeHtml(x)}</span>`).join("")}</div>`:""}</div></article>`).join("");
